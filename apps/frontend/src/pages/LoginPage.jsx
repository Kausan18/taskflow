    ...
    <div className="min-h-screen flex items-center justify-center bg-[#13161c] px-4">
      <div className="w-full max-w-md bg-[#1e2128] border border-[#2d3240] rounded-xl p-8 shadow-2xl animate-fade-in">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          ...
          {/* CAPTCHA notice after 3 failed attempts */}
          {failCount >= 3 && (
            <div className="rounded-md border border-yellow-500/30 bg-yellow-500/10 px-3 py-2.5 text-xs text-yellow-400">
              ⚠️ Too many failed attempts. A security check may be required.
            </div>
          )}

          {/* API error */}
          {authError && (
            <div className="flex items-center space-x-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2.5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full h-10 mt-1 flex justify-center items-center rounded-md bg-[#1a7a5e] hover:bg-[#14604a] text-white font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
            {isLoading ? (
        ...
        {/* Divider */}
        <div className="mt-5 mb-4 relative flex items-center">
          <div className="flex-1 border-t border-[#2d3240]" />
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex justify-center items-center h-10 rounded-md border border-[#2d3240] bg-transparent text-gray-300 hover:bg-[#242830] hover:border-[#3d4255] font-medium transition-colors disabled:opacity-50 text-sm"
        >
